...
// 从 URL 参数获取空间信息（实际应该从路由或 context 获取）
const spaceType = "personal"; // 可以是 'personal' 或 'enterprise'
    ...
    <div className="flex flex-col h-screen bg-[#f0f8ff]">
      ...
      <header className="flex-shrink-0 h-14 bg-white/80 backdrop-blur-xl border-b border-[#e2e8f0]/80 flex items-center justify-between px-6 z-10">
        <div className="flex items-center gap-4">
          {/* 返回按钮 - 融入系统导航 */}
          <a
            href={spaceType === "personal" ? "/workspace-hub" : "/workspace-hub?type=enterprise"}
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-slate-500">知阁·舟坊</span>
            <ChevronDown className="w-4 h-4 text-slate-400 rotate-[-90deg]" />
            <span className="text-slate-800 font-bold">Studio</span>
            <ChevronDown className="w-4 h-4 text-slate-400 rotate-[-90deg]" />
            <span className="text-slate-600">{currentSpace.name}</span>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* 下部：用户操作区 - 任务追踪与指标（也占 50% 高度） */}
        <div className="flex-1 min-h-0 bg-white/60 backdrop-blur-sm overflow-hidden flex flex-col">
          {/* 工具栏 */}
          <div className="flex-shrink-0 h-12 border-b border-[#e2e8f0]/80 flex items-center justify-between px-6">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 text-slate-800">
