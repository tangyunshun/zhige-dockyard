...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. 检查是否有进行中的组件任务
    const inProgressTasks = workspace.componentTasks.filter(t => t.status === "IN_PROGRESS");
    if (inProgressTasks.length > 0) {
      warnings.push(`空间有 ${inProgressTasks.length} 个进行中的组件任务，删除后将中断`);
    }

    // 3. 检查是否有已完成的组件
    const completedTasks = workspace.componentTasks.filter(t => t.status === "COMPLETED");
    if (completedTasks.length > 0) {
      warnings.push(`空间有 ${completedTasks.length} 个已完成的组件，删除后将不可用`);
    }
    ...
    return NextResponse.json({
      ...
      workspace: {
        ...
        componentCount: workspace.componentTasks.length,
      },
      issues,
      warnings,
    });
